...
if($user['role'] == 'administrator'){
    $sql = "SELECT * FROM videoposts";
    $result = $db->prepare($sql);
    $result->execute();
    $totalres = $result->rowCount();
}elseif($user['role'] == 'editor'){
    $sql = "SELECT * FROM videoposts WHERE uid=?";
    $result = $db->prepare($sql);
...
?>
        ...
        <div id="page-wrapper" style="min-height: 345px;">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header"> Welcome to View Video Post Section</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
  ...
  <!----------------------ADMIN CAN SEE ALL AND EDITOR CAN SEE ONLY HIS POST---------------------------------->
                                ...
                                <tr>
                                    ...
                                    <td><?php echo $post['news']; ?></td>
                                    <td><?php if(isset($post['video']) & !empty($post['video'])){ echo "Yes"; }else{ echo "No"; } ?></td>
                                    <td><?php echo $post['updated']; ?></td>
                                    <td><?php echo $post['status']; ?></td>
                                    <td><a href="edit-videopost.php?id=<?php echo $post['id']; ?>">Edit</a> | <a href="delete-item.php?id=<?php echo $post['id']; ?>&item=videopost">Delete</a></td>
                                </tr>
                                <?php } ?>
